...
$guestBaseline = Join-Path $root "scenarios/linux/silent-service/guest/baseline.sh"
...
$dockerApp = Join-Path $root "scenarios/docker/unreachable-database/assets/status-server.py"
...
Assert-Contains $guestBaseline "sleep 0\.25"
Assert-Contains $guestBaseline '>/dev/null 2>&1'
Assert-Contains $guestBaseline "Black Mesa systems operational"
...
Assert-Contains $dockerApp '"-q"'
Assert-Contains $dockerApp "return lines\[-1\]"
...
Assert-Contains $readme "lintendo.cmd machine doctor phoenix"
...
Assert-Contains $controller "machine doctor"
Assert-Contains $controller "Ssh-Executable"
Assert-Contains $controller "Incus daemon access"
Assert-Contains $controller "Storage available"
Assert-Contains $controller "Networking available"
Assert-Contains $controller "Runtime available"
